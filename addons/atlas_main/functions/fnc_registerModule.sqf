...
if (_name isEqualTo "") exitWith {
    ["Core", "ERROR", "registerModule: Empty module name"] call FUNC(log);
    false
};

if (GVAR(moduleRegistry) getOrDefault [_name, ""] isEqualType createHashMap) exitWith {
    ["Core", "WARN", format ["registerModule: Module '%1' already registered", _name]] call FUNC(log);
    false
};

// Validate dependencies
private _requires = _config getOrDefault ["requires", []];
private _missingDeps = _requires select { !(GVAR(moduleRegistry) getOrDefault [_x, ""] isEqualType createHashMap) };

if (count _missingDeps > 0) then {
    ["Core", "WARN", format ["registerModule: Module '%1' has unmet dependencies: %2 (may load later)", _name, _missingDeps]] call FUNC(log);
};

// Register
_config set ["_registeredAt", diag_tickTime];
GVAR(moduleRegistry) set [_name, _config];

["Core", "INFO", format ["Module registered: %1 v%2", _name, _config getOrDefault ["version", "unknown"]]] call FUNC(log);

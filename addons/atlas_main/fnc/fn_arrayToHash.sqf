...
if !(_array isEqualType []) exitWith {
    ["Core", "ERROR", format ["arrayToHash: Expected array, got %1", typeName _array]] call ATLAS_fnc_log;
    createHashMap
};
...
if !((_array#0) isEqualType []) exitWith {
    ["Core", "ERROR", "arrayToHash: Array elements must be [key, value] pairs"] call ATLAS_fnc_log;
    createHashMap
};
...

{
    ...
    if (_val isEqualType [] && {count _val > 0} && {(_val#0) isEqualType []} && {count (_val#0) == 2} && {((_val#0)#0) isEqualType ""}) then {
        _val = [_val] call ATLAS_fnc_arrayToHash;
    };
